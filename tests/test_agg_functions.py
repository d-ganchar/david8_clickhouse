...
class TestAggFunctions(BaseTest):
    ...
    @parameterized.expand([
        (
            uniq('user_id').over(partition_by=['host'], order_by=['ts']),
            'SELECT uniq(user_id) OVER (PARTITION BY host ORDER BY ts)',
        ),
    ])
    def test_uniq(self, fn: FunctionProtocol, exp_sql: str):
        query = self.qb.select(fn)
        self.assertEqual(query.get_sql(), exp_sql)
